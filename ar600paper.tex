...
moveIt support for whole robot and hand assemblies, gazebo support((Fig.
\ref{img:moveitgazebo})), vision, speech packets and set of drivers for direct
interaction with robot.
Right now movement groups for hands, legs and head are available via
MoveIt, PID-controllers available via ros-control
interfaces. And we already have objects detector built over cascades and OpenCV.
